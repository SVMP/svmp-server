 ...
 */
function destroyVM (obj, callback) {
    var deferred = Q.defer();
...
exports.destroyVM = destroyVM; // export the function to be used from outside the module
 ...
 * @returns a Promise with the request object
 */
exports.restartVM = function (obj) {
    var deferred = Q.defer();

    computeClient.rebootServer(obj.vm_id, {type: 'HARD'}, function(err) {
        if (err) {
            destroyAndReject(obj.vm_id, deferred, "restartVM failed", err);
        } else {
            winston.verbose("Restarted VM, ID '%s'", obj.vm_id);
            deferred.resolve(obj);
        }
    });
