    ...
    computeClient.createServer(vmObj, function (err, server) {
        if (err) {
            ...
            server.setWait({status: server.STATUS.running}, settings.new_vm_defaults.pollintervalforstartup, function (err) {
                if (err) {
                    deferred.reject(new Error('createAndStartVM failed, waiting for startup: ' + err.message));
                } else {
                    ...
                    if (settings.new_vm_defaults.use_floating_ips) {
                        ...
                        if (server.addresses && server.addresses.private && server.addresses.private.length > 0) {
                            ...
                            exports.destroyVM({vm_id: user.vm_id})
                                .fin(function() {
                                    deferred.reject(new Error('createAndStartVM failed: cannot find IP of new VM'));
                                });
                        }
                    }
                }
...
exports.attachVolumeToVMForUser = function (user) {
    ...
    computeClient.attachVolume(user.vm_id, user.volume_id, function (err, result) {
        if (err) {
            ...
            exports.destroyVM({vm_id: user.vm_id})
                .fin(function() {
                    deferred.reject(new Error('attachVolumeToVMForUser failed: ' + err.message));
                });
        } else {
            deferred.resolve(user);
        }
    ...
    if (settings.new_vm_defaults.use_floating_ips) {
        ...
        computeClient.allocateNewFloatingIp(pool, function(err, ip) {
            if (err) {
                ...
                exports.destroyVM({vm_id: user.vm_id})
                    .fin(function() {
                        deferred.reject(new Error("assignFloatingIpToVM failed, couldn't allocate address: " + err.message));
                    });
            } else {
                ...
                computeClient.addFloatingIp(user.vm_id, user.vm_ip, function(err) {
                    if (err) {
                        // we allocated an IP but failed to assign it to a VM; deallocate it
                        computeClient.deallocateFloatingIp(user.vm_ip, function(err) {
                            // a VM was created; destroy it, then throw an error
                            exports.destroyVM({vm_id: user.vm_id})
                                .fin(function() {
                                    deferred.reject(new Error("assignFloatingIpToVM failed, couldn't add address to VM: " + err.message));
                                });
                        });
                    } else {
                        // success
