 ...
 */
Configuration.prototype.configOverseer = function () {
    var overseerCertPath = this.get('settings:svmp_overseer_cert');

    // attempt to read SVMP Overseer certificate (used to decode JWTs)
    try {
        var overseerCert = fs.readFileSync(overseerCertPath);
        this.set('overseerCert', overseerCert);
    } catch (err) {
...
Configuration.prototype.getVideoResponse = function() {
    // Stringify parameters
    var ice = JSON.stringify(this.get('webrtc:ice'));
    var video = JSON.stringify(this.get('webrtc:video'));
    var pc = JSON.stringify(this.get('webrtc:pc'));
 ...
 * @returns {}
 */
Configuration.prototype.getRestParams = function (path, method, body) {
    var value = {
        'host': this.get('settings:svmp_overseer_host'),
        'port': this.get('settings:svmp_overseer_port'),
        'ssl': this.get('settings:tls_proxy'),
        'headers': {
            'svmp-authtoken': this.get('settings:svmp_auth_token')
        },
        'path': path
    };
