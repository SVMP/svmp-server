...
nconf.init = function (configFile) {
    ...
    if (!validation.valid) {
        validation.errors.forEach(function(e) {
            svmp.logger.error('Error parsing config file: %s %s', e.property, e.message);
        });
        process.exit(1);
    } else {
...
nconf.configTls = function () {
    if (svmp.config.isEnabled('enable_ssl')) {
        ...
        try {
            ...
            svmp.logger.error("Could not open TLS private key '%s'", privateKeyPath);
            process.exit(1);
        }
        try {
            var tls_cert = fs.readFileSync(certFilePath);
        } catch (err) {
            svmp.logger.error("Could not open TLS certificate '%s'", certFilePath);
            process.exit(1);
        }
        options.type = 'tls';
        options.key = tls_key;
        ...
        if (svmp.config.isEnabled('cert_user_auth')) {
            ...
            try {
                ...
                svmp.logger.error("Could not open TLS ca cert file '%s'", cacertPath);
                process.exit(1);
            }
...
nconf.configOverseer = function () {
    ...
    try {
        ...
        svmp.logger.error("Could not open overseer cert file '%s'", overseerCertPath);
        process.exit(1);
    }
};
