...
var ProxyUserSchema = new Schema({
    ...
    password_change_needed: Boolean, // Optional, true if user needs to reset password
    vm_id: String,
    vm_ip: String,
...
exports.changeUserPassword = function(obj) {
    var deferred = Q.defer();
    var query = {username: obj.user.username};
    var update = {password: obj.requestObj.newPassword, password_change_needed: false};
    Model.update(query, update, function (err, numberAffected, raw) {
        if (err) {
            deferred.reject(new Error("Could not change password for user '" + obj.username + "': " + err));
        } else {
...
exports.createUser = function (username, password, device_type, callback) {
    ...
    var user = new Model({username: username, password: password, password_change_needed: true, device_type: device_type, volume_id: ''});
    user.save(callback);
};
