...
openstack.getFlavors = function (callback) {
    this.computeClient.getFlavors(function (err, flavors) {
        var list = [];
        if (err) {
            callback(err);
        } else {
...
openstack.getImages = function (callback) {
    this.computeClient.getImages(function (err, images) {
        var list = [];
        if (err) {
            callback(err);
        } else {
...
openstack.createAndStartVM = function (user, image, flavor) {
    ...
    this.computeClient.createServer(vmObj, function (err, server) {
...
openstack.createVolumeForUser = function (user) {
    ...
    this.blockClient.createVolume(opts, function (err, vol) {
        if (err) {
...
openstack.attachVolumeToVMForUser = function (user) {
    var deferred = Q.defer();
    this.computeClient.attachVolume(user.vm_id, user.volume_id, function (err, result) {
        if (err) {
...
openstack.assignFloatingIpToVM = function (user) {
    ...
    if (svmp.config.isEnabled('settings:new_vm_defaults:use_floating_ips')) {
        ...
        this.computeClient.allocateNewFloatingIp(pool, function (err, ip) {
            if (err) {
                that.destroyAndReject(user.vm_id, deferred, "assignFloatingIpToVM failed, couldn't allocate address", err);
            } else {
                ...
                that.computeClient.addFloatingIp(user.vm_id, user.vm_ip, function (err) {
                    if (err) {
                        that.destroyAndReject(user.vm_id, deferred, "assignFloatingIpToVM failed, couldn't add address to VM", err);
                    } else {
                        // success
                        deferred.resolve(user);
                    }
...
function deallocateFloatingIp(user) {
    ...
    this.computeClient.deallocateFloatingIp(user.vm_ip_id, function (err) {
        if (err) {
...
openstack.destroyVM = function (obj, callback) {
    ...
    if (obj.vm_id) {
        this.computeClient.destroyServer(obj.vm_id, function (err, id) {
            if (err) {
...
openstack.restartVM = function (obj) {
    ...
    this.computeClient.rebootServer(obj.vm_id, {type: 'HARD'}, function (err) {
        if (err) {
            that.destroyAndReject(obj.vm_id, deferred, "restartVM failed", err);
        } else {
 ...
 */
openstack.destroyAndReject = function (vm_id, deferred, message, err) {
    // a VM was created; destroy it, then throw an error
    this.destroyVM({'vm_id': vm_id})
        .fin(function () {
