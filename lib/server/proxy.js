    ...
    .then(function (obj) {
        ...
        .then(function (response) {
            ...
            clientSocket.info.vmSocket.connect(user.vm_port, user.vm_ip, function () {
                ...
                svmp.logger.debug("User '%s' session state changed to VMREADY_WAIT");
            });
        }).catch( function (err) {
//            svmp.logger.err('Error connecting to VM at %s: %s',
//                user.vm_ip, err.message);
            clientSocket.response({type: 'ERROR'});
            clientSocket.close(4001, 'setup.onLogin failed: ' + err.message);
        }).done();

    }).catch( function (err) {
        svmp.logger.info("Client %s:%d failed authentication: %s",
    ...
    clientSocket.on("close", function (code, message) {
        svmp.logger.verbose("clientSocket closed: (%d) %s", code, message);
        shutdownClient();
    });
    ...
    clientSocket.info.vmSocket.on('close', function (had_error) {
        svmp.logger.verbose("Connection to VM %s closed",
    ...
    clientSocket.info.vmSocket.on('timeout', function () {
        svmp.logger.verbose("Connection to VM %s timed out",
            clientSocket.info.vmAddress);
        shutdownVM();
    });
