...
exports.handleConnection = function (clientSocket) {
    ...
    .then(function (useSessionObj) {
        ...
        // XXX are we sending AUTH OK or skipping straight to VMREADY?
        var msg = {type: 'AUTH', authResponse: {type: "AUTH_OK" } };
        clientSocket.send(svmp.protocol.writeResponse(msg), {binary: true});

    // 2) Open the connection to the VM
    // XXX this is mostly old code from here until clientSocket.on('message')
        ...
        .then(function (useSessionObj) {
            ...
            clientSocket.send(svmp.protocol.writeResponse({type: 'ERROR', message: err}), {binary: true});
            //clientSocket.sendResponse({type: 'ERROR', message: err});
            clientSocket.close(4001, err);
        }).done();

    }, function (err) {
        svmp.logger.info('Failed authentication: ' + err.message);
        clientSocket.send(svmp.protocol.writeResponse({type: 'AUTH', authResponse: {type: err.responseType}}), {binary: true});
        clientSocket.close(4002, err.message);
    }).done();
        ...
        sessionInterval = setInterval(
            function interval() {
                ...
                if (expired) {
                    ...
                    clientSocket.send(svmp.protocol.writeResponse({type: "AUTH", authResponse: {type: "SESSION_MAX_TIMEOUT"}}),
                                      {binary: true});
