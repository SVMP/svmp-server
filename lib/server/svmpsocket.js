 ...
 */
PacketReader.prototype.readBuffer = function (buff) {
    var that = this;
    ...
    var handleData = function (buff) {
        ...
        // New packet start
        if (that.expectedSize < 0) {
            var m = readVarIntHeader(buff);
            that.expectedSize = m.value  + m.length;
        }
        var expectedRemaining = that.expectedSize - that.receivedSize;
        if (buff.length > expectedRemaining) {
            ...
            that.incomingPacket.push(newBuff);
            that.receivedSize = that.expectedSize;
        } else {
            that.incomingPacket.push(buff);
            that.receivedSize += buff.length;
        }

        if (that.receivedSize == that.expectedSize) {
            // Forward the message
            that.callback(Buffer.concat(that.incomingPacket));
...
SvmpSocket.prototype.getPeerCertificate = function () {
    if (!this.socket) {
        return this.emit('error', new Error('SvmpSocket: socket is broken'));
    }
    ...
    try {
        ...
        svmp.logger.error("Error getting client certificate: " + e);
    }
    return cert;
};

SvmpSocket.prototype.isAuthorized = function () {
    if (!this.socket) {
        return this.emit('error', new Error('SvmpSocket: socket is broken'));
    }
