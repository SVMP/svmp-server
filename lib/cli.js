...
// Connect to mongoose
// If you change this - change the call in server.js as well
...
program
    ...
    .action(function () {
        ...
        console.log('');
        var images = settings.new_vm_defaults.images;
...
program
    ...
    .action(function (un, pw, dev) {
        ...
        if (!dev || dev.length == 0 || !images.hasOwnProperty(dev)) {
            console.log("Problem creating user: device not recognized (run ./bin/spm devices to see available devices)".error);
            mongoose.connection.close();
        }
        else {
            users.createUser(un, pw, dev, function (err, u) {
                if (u) {
...
program
    ...
    .action(function () {
        ...
        Openstack.getFlavors(function (err, allflavors) {
            if (err) {
                console.log("ERROR: ", err);
            } else {
