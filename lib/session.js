...
var SessionSchema = new Schema({
    ...
    expireAt: {type: Date, expires: config.session_token_ttl}
});
...
exports.create = function (obj) {
    ...
    var s = new SessionModel({sid: uidhelper(UID_SIZE), username: obj.username, expireAt: new Date()});
...
exports.testSession = function(id, username, callback) {
    var s = new SessionModel({sid: id, username: username, expireAt: new Date()});
    s.save(function(err, sess){
        if(err) { 
...
/**
 * Get a session. Update expiration if present
 * @param {Object} raw request object from client
 * @returns Promise with an Object {sid,uid}
 */
exports.get = function (requestObj) {
    var deferred = Q.defer();
    SessionModel.findOne({sid: requestObj.sessionToken}, function(err, session) {
        if(session) {
            session.expireAt = new Date();
	        session.save(function(err, sess){
                if(sess) {
                    deferred.resolve({sid: sess.sid, username: sess.username});   
                } else {
                    deferred.reject(requestObj);
                } 
            });
	   } else {
           deferred.reject(requestObj);
